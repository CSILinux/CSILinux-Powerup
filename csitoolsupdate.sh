...
setup_new_csi_user_and_system() {
    ...
    echo $key | sudo -S apt remove --purge --allow-remove-essential -y $(dpkg --get-selections | awk '/i386/{print $1}') > /dev/null 2>&1
...
echo $key | sudo -S add-apt-repository --no-update ppa:apt/stable -y > /dev/null 2>&1
echo $key | sudo -S add-apt-repository --no-update  ppa:obsproject/obs-studio -y > /dev/null 2>&1

echo $key | sudo -S apt install aria2 apt -y > /dev/null 2>&1
echo $key | sudo -S apt install bash-completion > /dev/null 2>&1
echo $key | sudo -S cp completions/bash/apt /etc/bash_completion.d/ > /dev/null 2>&1
echo $key | sudo -S chown root:root /etc/bash_completion.d/apt > /dev/null 2>&1
. /etc/bash_completion
